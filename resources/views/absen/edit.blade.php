@extends('layout.happy')
@section('content')

<div class="container my-5">
	<a href="/absen"> <i class="fas fa-arrow-left"> </i></a>
    <br/>
	<br/>
    <h3> Edit Status </h3>
	<br/>

	@foreach($absen as $a)
	<form action="/absen/update" method="post" class="container-fluid">
		{{ csrf_field() }}
        <div class="row mb-3">
            <label class="col-sm-2 col-form-label""></label>
        <div class="col-3">
		    <input type="hidden" name="id" class="form-control" value="{{ $a->ID }}">
        </div>
        </div>

        <div class="row mb-3">
            <label for="nama" class="col-sm-2 col-form-label">Nama Pegawai </label>
            <div class="col-3" id='nama'>
                <select class="form-control" name="idpegawai">
                    @foreach($pegawai as $p )
                        <option value="{{ $p->pegawai_id }}" @if($p->pegawai_id===$a->IDPegawai) selected="selected" @endif> {{ $p->pegawai_nama }}</option>
                    @endforeach
                </select>
            </div>
        </div>

        <div class="row mb-3">
            <label for="dtpickerdemo" class="col-sm-2 col-form-label">Tanggal </label>
            <div class='col-3 input-group date'>
                <input type="datetime-local" class="form-control" name="tanggal" required="required" value="{{ date('Y-m-d\TH:i', strtotime($a->Tanggal)) }}" />
            </div>
        </div>

        <div class="row mb-3">
            <label class="col-sm-2 col-form-label">Status</label>
            <div class="col-3">
            <div class="form-check">
                <input class="form-check-input" type="radio" name="status" id="h" value="H" @if($a->Status==='H') checked="checked" @endif>
                <label class="form-check-label" for="h">
                    Hadir
                </label>
              </div>
              <div class="form-check">
                <input class="form-check-input" type="radio" name="status" id="a" value="A" @if($a->Status==='A') checked="checked" @endif>
                <label class="form-check-label" for="a">
                    Tidak Hadir
                </label>
              </div>
            </div>
        </div>

        <div class="row mt-4 mb-5">
            <label class="col-sm-2 col-form-label"></label>
            <div class="col-3">
                <button type="submit" class=" form-control btn-success">Simpan Data</button>
            </div>
        </div>

	</form>
	@endforeach
</div>
@endsection

